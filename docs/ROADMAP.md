...
3. CB Source-Contract Hardening & Redemption Observability
   ...
   - **Redemption Wave 3** *(Complete)*: Integrate event ledger / shared-state for true announcement-driven `redeem_risk` hydration. The contract boundaries for the ingress artifact, persisted ledger, trace artifact, and canonical state are now strictly defined and frozen. Phase 1.5 (Wave 3) is complete, explicitly noting that updater scheduling (Wave 5) and rollout (Wave 6) remain pending.
