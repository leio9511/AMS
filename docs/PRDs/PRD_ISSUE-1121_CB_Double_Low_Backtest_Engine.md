...
- **Data Layer (`cb_data_loader.py`)**: A script designed to run on the Windows QMT node (or fetch via SSH).
  - **Historical Premium & Scale Resolution**: To completely avoid survivorship bias and forward-looking functions, the script MUST integrate with `akshare`'s `ak.bond_zh_hs_cov_daily` (or equivalent API that provides historical outstanding scales and premium rates without lookahead bias). It must correctly join these historical daily fundamental snapshots with QMT OHLCV or use the accurate Akshare daily historical data directly if QMT cannot provide historical constituent data for delisted bonds.
  - The script will reconstruct the PiT daily premium rates (`close / (100 / pit_conversion_price * stock_close) - 1`) and save a flattened historical dataset (e.g., Parquet or JSON).
