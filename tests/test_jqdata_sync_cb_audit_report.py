...
def test_validator_summary_maps_existing_validator_outcomes_without_copying_rules(mock_env, mock_jqdata, mock_validators, isolated_paths):
    ...
    # Force semantic validator failure with a message; this exercises the legacy
    # direct DatasetSemanticValidator compatibility path, not Stage F.
    ...
    assert report["supportability_summary"]["supportable_row_count"] > 0
    assert report["validator_summary"]["core_validator_status"] == STAGE_STATUS_PASS
    assert report["validator_summary"]["failure_type"] == "NONE"
    assert "Custom Semantic Error" not in report["validator_summary"]["core_validator_message"]
    mock_v2.return_value.validate_dataframe.assert_not_called()
