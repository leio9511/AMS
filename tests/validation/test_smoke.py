...
def test_sensitivity_sanity_sl():
    """
    Scenario 5: Stop-loss sensitivity sanity remains alive.
    Summary results must change when sl is modified.
    """
    base_args = [
        sys.executable, "main_runner.py",
        "--strategy", "cb_rotation",
    ...
    # Case 1: sl = -0.10
    res1 = subprocess.run(base_args + ["--sl", "-0.10"], capture_output=True, text=True)
    summary1 = json.loads(res1.stdout)["summary"]
    
    # Case 2: sl = -0.05
    res2 = subprocess.run(base_args + ["--sl", "-0.05"], capture_output=True, text=True)
    summary2 = json.loads(res2.stdout)["summary"]
    
    # Check that they are not identical
    different = any(summary1[k] != summary2[k] for k in ["total_return", "max_drawdown", "calmar_ratio", "final_equity"])
    assert different, "Summary should change when SL is modified"

def test_sensitivity_sanity_tp_pos():
    """
    ...
    # Case 1: tp_pos = 0.15
    res1 = subprocess.run(base_args + ["--tp-pos", "0.15"], capture_output=True, text=True)
    summary1 = json.loads(res1.stdout)["summary"]
    
    # Case 2: tp_pos = 0.25
    res2 = subprocess.run(base_args + ["--tp-pos", "0.25"], capture_output=True, text=True)
    summary2 = json.loads(res2.stdout)["summary"]
    
    different = any(summary1[k] != summary2[k] for k in ["total_return", "max_drawdown", "calmar_ratio", "final_equity"])
    assert different, "Summary should change when tp_pos is modified"

def test_sensitivity_sanity_tp_intra():
    """
    ...
    # Case 1: tp_intra = 0.12
    res1 = subprocess.run(base_args + ["--tp-intra", "0.12"], capture_output=True, text=True)
    summary1 = json.loads(res1.stdout)["summary"]
    
    # Case 2: tp_intra = 0.08
    res2 = subprocess.run(base_args + ["--tp-intra", "0.08"], capture_output=True, text=True)
    summary2 = json.loads(res2.stdout)["summary"]
    
    different = any(summary1[k] != summary2[k] for k in ["total_return", "max_drawdown", "calmar_ratio", "final_equity"])
    assert different, "Summary should change when tp_intra is modified"
